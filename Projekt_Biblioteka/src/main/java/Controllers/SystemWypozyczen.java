    ...
    void dodaj_action(ActionEvent event)  {
        ...
        if (ksiazkaBox.getValue() == null || czytelnikBox.getValue()==null || wypozyczenieDate.getValue()==null ) {
            ...
            try {
                ...
                CallableStatement cstmt = con.prepareCall("{call WYPOZYCZENIA_TAPI.INSERTISSUE(?,?,?,?)}");
...
//        ResultSet rs = pst.executeQuery();
        CallableStatement cstmt = con.prepareCall("{ call WYPOZYCZENIA_TAPI.INFOKSIAZKA(?,?)}");
...
//
//        ResultSet rs = pst.executeQuery();
        CallableStatement cstmt = con.prepareCall("{ call WYPOZYCZENIA_TAPI.LIST1WYPOZYCZENIA(?)}");
        cstmt.registerOutParameter(1, OracleTypes.CURSOR);
        cstmt.execute();
        ResultSet rs = (ResultSet) cstmt.getObject(1);
