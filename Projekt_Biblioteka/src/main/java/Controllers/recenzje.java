    ...
    void dodajAction(ActionEvent event) {
        ...
        if (ksiazkaBox.getValue() == null || ocena == 0 || areaComment.getText().isEmpty()) {
            ...
            try {
                ...
                CallableStatement cstmt = con.prepareCall("{call RECENZJE_TAPI.INSERTREVIEW(?,?,?,?,?)}");
                cstmt.setInt(1, id);
...
//        ResultSet rs = pst.executeQuery();
        CallableStatement cstmt = con.prepareCall("{ call RECENZJE_TAPI.LIST1RECENZJE(?,?)}");
