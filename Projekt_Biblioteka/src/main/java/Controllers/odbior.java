    ...
    void odbierzAction(ActionEvent event) {
        if (zamowienieBox.getValue() == null || czytelnikBox.getValue()==null || zwrotDate.getValue()==null ) {
            ...
            try {
                ...
                CallableStatement cstmt = con.prepareCall("{call ODBIOR_TAPI.PICKORDER(?,?,?)}");
                cstmt.setInt(1, id);
                cstmt.setInt(2, id2);
...
//        ResultSet rs = pst.executeQuery();
        CallableStatement cstmt = con.prepareCall("{ call ODBIOR_TAPI.CZYTELNICYODBIOR(?)}");
    ...
    public ObservableList<zamowienie> Zamowienia() throws ClassNotFoundException, SQLException {
        ...
        CallableStatement cstmt = con.prepareCall("{ call ODBIOR_TAPI.ZAMOWIENIAODBIOR(?,?)}");
        cstmt.registerOutParameter(1, OracleTypes.CURSOR);
        cstmt.setInt(2,id);
        cstmt.execute();
        ResultSet rs = (ResultSet) cstmt.getObject(1);
...
//        pst.setInt(2,id2);
//        ResultSet rs = pst.executeQuery();
        CallableStatement cstmt = con.prepareCall("{ call ODBIOR_TAPI.INFOODBIOR(?,?,?)}");
        cstmt.registerOutParameter(1, OracleTypes.CURSOR);
        cstmt.setInt(2,id);
        cstmt.setInt(3,id2);
        cstmt.execute();
